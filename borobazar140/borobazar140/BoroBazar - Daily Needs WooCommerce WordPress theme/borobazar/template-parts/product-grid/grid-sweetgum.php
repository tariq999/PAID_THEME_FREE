...
<div <?php wc_product_class('borobazar-grid-oak', $product); ?>>
    ...
    <div class="borobazar-product-<?php echo esc_attr($product->get_id()); ?> borobazar-oak-product-card grid grid-rows-[max-content] h-full border border-[#EAEEF2] rounded-lg bg-white overflow-hidden transition-all hover:shadow-product-hover">
        ...
        <div class="borobazar-oak-product-card-thumb relative h-40 sm:h-48 overflow-hidden">
            <a class="borobazar-image-fade-in" href="<?php echo esc_url($link); ?>" aria-label="<?php echo esc_attr($productTitle); ?>">
                ...
                <?php } ?>
                <?php if (!$isInStock) { ?>
                    <span class="product-badge text-[11px] text-white font-bold bg-error rounded-[4px] py-0.5 px-1.5 absolute top-3 sm:top-3.5 right-3 sm:right-3.5 z-1">
                        <?php echo esc_html__('Out Of Stock', 'borobazar'); ?>
                    </span>
                <?php } ?>

                <?php if ($isInStock && $isOnSale && $productType === 'simple') { ?>
                    <span class="product-badge text-[11px] text-white font-bold bg-brand rounded-[4px] py-0.5 px-1.5 absolute top-3 sm:top-3.5 right-3 sm:right-3.5 z-1">
        ...
        <div class="pt-2.5 sm:pt-3 flex flex-col justify-between">
            <a href="<?php echo esc_url($link); ?>" class="px-3 sm:px-4 borobazar-oak-product-card-title block mb-auto text-sm sm:text-md leading-6 overflow-hidden text-ellipsis no-underline">
                <?php woocommerce_template_loop_product_title(); ?>
            </a>
            <div class="px-3 sm:px-4 borobazar-oak-product-card-price text-sm sm:text-base text-main font-semibold mb-0.5 sm:mb-1.5">
            ...
            <?php if ($isInStock) { ?>
                <?php if ($productType === 'simple') { ?>
                    ...
                    <!-- Add to cart button -->
                    <a href="<?php echo esc_url($product_link); ?>" class="<?php echo esc_attr($cartButton); ?> sweetgum-quick-view-button grid grid-cols-[1fr,max-content] items-center mt-[10px] no-underline transition-all text-black font-medium product_type_simple borobazar-update-qty borobazar-add-to-cart-<?php echo esc_attr($product->get_id()); ?>" data-product_id="<?php echo esc_attr($product->get_id()); ?>" aria-label="<?php echo esc_attr__('Add to Cart', 'borobazar'); ?>">
                        <span class="flex items-center justify-center uppercase"> <?php echo esc_html__('Add to cart', 'borobazar'); ?> </span>
                        <span class="sweetgum-increment-button w-10 h-10 rounded-tr-[4px] rounded-br-[4px] flex items-center justify-center ml-auto">
                    ...
                    <!-- Counter -->
                    <div class="<?php echo esc_attr($display); ?> sweetgum-quick-view-button sweetgum-counter justify-between z-1 mt-[10px] borobazar-qty-button borobazar-qty-button-<?php echo esc_attr($product->get_id()); ?>">

                        <span class="border-right flex items-center justify-center w-10 h-10 cursor-pointer transition-all decrement borobazar-update-qty" data-product_id="<?php echo esc_attr($product->get_id()); ?>" data-type="minus" data-stock_qty="<?php echo esc_attr($stock_qty); ?>" title="<?php echo esc_attr__('Decrement', 'borobazar'); ?>">
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <span class="sweetgum-product-quantity self-center text-sm sm:text-base text-black font-semibold quantity borobazar-cart-qty borobazar-cart-product-<?php echo esc_attr($product->get_id()); ?>">
                            <?php echo wp_kses('x' . $qty, $allowedHTML); ?>
                        </span>

                        <span class="sweetgum-increment-button flex items-center justify-center w-10 h-10 cursor-pointer transition-all increment borobazar-update-qty" data-product_id="<?php echo esc_attr($product->get_id()); ?>" data-type="plus" data-stock_qty="<?php echo esc_attr($stock_qty); ?>" title="<?php echo esc_attr__('Increment', 'borobazar'); ?>">
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M14.2999 7.29998H8.70002V1.69996C8.70002 1.31364 8.38638 1 7.99994 1C7.61362 1 7.29998 1.31364 7.29998 1.69996V7.29998H1.69996C1.31364 7.29998 1 7.61362 1 7.99994C1 8.38638 1.31364 8.70002 1.69996 8.70002H7.29998V14.2999C7.29998 14.6864 7.61362 15 7.99994 15C8.38638 15 8.70002 14.6864 8.70002 14.2999V8.70002H14.2999C14.6864 8.70002 15 8.38638 15 7.99994C15 7.61362 14.6864 7.29998 14.2999 7.29998Z" fill="black" stroke="black" stroke-width="0.5" />
                            </svg>
                        </span>
                ...
                <?php } else { ?>
                    ...
                    <?php if (($showQuickView === 'on' && class_exists('RedQWooCommerceQuickView')) && $productType !== 'redq_rental') { ?>
                        <a href="#redq-quick-view-modal" class="sweetgum-quick-view-button button-redq-woocommerce-quick-view grid grid-cols-[1fr,max-content] items-center mt-[10px] no-underline transition-all text-black font-medium" data-product_id="<?php echo esc_attr($product->get_id()); ?>" aria-label="<?php echo esc_attr__('View Products', 'borobazar'); ?>" rel="modal:open">
                            <span class="flex items-center justify-center uppercase"><?php echo esc_html__('View Products', 'borobazar'); ?></span>
                            <span class="sweetgum-increment-button w-10 h-10 rounded-tr-[4px] rounded-br-[4px] flex items-center justify-center ml-auto">
                                <svg stroke="black" fill="none" stroke-width="2.1" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" height="1.25em" width="1.25em" xmlns="http://www.w3.org/2000/svg">
