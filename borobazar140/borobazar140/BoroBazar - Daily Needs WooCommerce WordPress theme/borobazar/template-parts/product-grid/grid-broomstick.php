...
<div <?php wc_product_class('borobazar-grid-maple', $product); ?>>
    ...
    <div class="borobazar-product-<?php echo esc_attr($product->get_id()); ?> borobazar-broomstick-product-card h-full grid grid-cols-[160px,1fr] xl:grid-cols-[180px,1fr] 4xl:grid-cols-[220px,1fr] border border-[#EAEEF2] bg-white overflow-hidden transition-all hover:shadow-product-hover">

        <div class="borobazar-alpine-product-card-thumb border-r border-[#EAEAEA] relative h-48 overflow-hidden">
            <a href="<?php echo esc_url($link); ?>" class="borobazar-image-fade-in block" aria-label="<?php echo esc_attr($productTitle); ?>">
                ...
                <?php } ?>
                <?php if (!$isInStock) { ?>
                    <span class="py-[2px] px-2 bg-error text-white text-[12px] uppercase rounded-full absolute top-[10px] right-[10px] z-1">
        ...
        </div> <!-- end .borobazar-alpine-product-card-thumb -->

        <div class="borobazar-alpine-product-card-meta flex flex-col pt-[18px]">
            <div class="pl-5 pr-4">

                <a href="<?php echo esc_url($link); ?>" class="borobazar-alpine-product-card-title block text-sm sm:text-base font-medium leading-6 overflow-hidden text-ellipsis no-underline">
                    <?php woocommerce_template_loop_product_title(); ?>
                </a>

                <div class="borobazar-alpine-product-card-price text-main font-bold mb-0.5 sm:mb-1.5">
            ...
            <?php if ($isInStock) { ?>
                <?php if ($productType === 'simple') { ?>
                    ...
                    <!-- Add to cart button -->
                    <a href="<?php echo esc_url($product_link); ?>" class="<?php echo esc_attr($cartButton); ?> flex justify-between items-center mt-auto no-underline transition-all text-gray-600 hover:text-black font-medium product_type_simple borobazar-update-qty borobazar-add-to-cart-<?php echo esc_attr($product->get_id()); ?>" data-product_id="<?php echo esc_attr($product->get_id()); ?>" aria-label="<?php echo esc_attr__('Add to Cart', 'borobazar'); ?>">
                        <span class="flex items-center justify-center transition-all decrement borobazar-update-qty borobazar-update-qty-minus" data-product_id="<%= list.post_id %>" title="<?php echo esc_attr__('Decrement', 'borobazar'); ?>"">
                        ...
                        <span class="borobazar-update-qty-plus flex items-center justify-center">
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <!-- Counter -->
                    <div class="<?php echo esc_attr($display); ?> justify-between z-1 mt-auto border-t border-[#F4F6F8] borobazar-qty-button borobazar-qty-button-<?php echo esc_attr($product->get_id()); ?>">
                        <span class="borobazar-update-qty-minus active-qty flex items-center justify-center border-r border-[#F4F6F8] w-12 h-12 cursor-pointer transition-all decrement borobazar-update-qty" data-product_id="<?php echo esc_attr($product->get_id()); ?>" data-type="minus" data-stock_qty="<?php echo esc_attr($stock_qty); ?>" title="<?php echo esc_attr__('Decrement', 'borobazar'); ?>">
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_3845_18)">
                ...
                <?php } else { ?>
                    ...
                    <?php } else { ?>
                        <a href="<?php echo esc_url($link); ?>" class="grid grid-cols-[1fr,max-content] items-center bg-[#F4F6F8] rounded-[4px] mt-auto no-underline transition-all text-gray-600 hover:text-black font-medium" data-product_id="<?php echo esc_attr($product->get_id()); ?>" aria-label="<?php echo esc_attr__('View Products', 'borobazar'); ?>">
                            <span class="flex items-center justify-center"><?php echo esc_html__('View Products', 'borobazar'); ?></span>
                            <span class="w-10 h-10 bg-[#E5E8EC] rounded-tr-[4px] rounded-br-[4px] flex items-center justify-center ml-auto">
                ...
                <?php } ?>
            <?php } ?>
        </div> <!-- end .borobazar-alpine-product-card-meta -->
